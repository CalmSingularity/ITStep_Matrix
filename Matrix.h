...
public:
	...
	double operator ()(size_t h, size_t w);  // Returns a single element
	void setElement(size_t h, size_t w, double value);  // Assigns the value to the data[h][w]

	Matrix getMultipliedBy(double scalar);   // Returns matrix multiplied by a scalar
	Matrix getRaisedTo(int power);   // Returns matrix raised into power
	double calculateTrace();    // Calculates and returns a trace of the matrix
